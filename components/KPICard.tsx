...
export default function KPICard({ title, value, trend, isGoodTrend, trendLabel, sparkline, color, delay = 0 }: KPICardProps) {
  const arrow = trend === 'up' ? '↑' : trend === 'down' ? '↓' : '–'
  const trendColor = trend === 'neutral'
    ? 'text-gray-400 dark:text-white/35'
    : isGoodTrend ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <span className="text-xs font-medium text-gray-500 dark:text-white/50 uppercase tracking-wider">{title}</span>
        {trendLabel && (
          <span className={`text-xs font-semibold ${trendColor} flex items-center gap-0.5`}>
            <span>{arrow}</span><span>{trendLabel}</span>
          </span>
        )}
      </div>
