...
export default function Section3Customers({ data }: { data: DashboardData }) {
  ...
  const cityData = {
    ...
    datasets: [{
      ...
      backgroundColor: data.revenueByCity.map((_, i) => i === 0 ? `${COLORS[0]}dd` : `${COLORS[0]}55`),
      borderColor: data.revenueByCity.map((_, i) => i === 0 ? COLORS[0] : `${COLORS[0]}88`),
      borderWidth: 1, borderRadius: 6, hoverBackgroundColor: COLORS[0],
    }],
  }
  ...
  const stackedData = {
    ...
    datasets: [
      { label: 'Online',   data: data.channelByCity.map(c => c.online),   backgroundColor: `${COLORS[0]}99`, borderColor: COLORS[0], borderWidth: 1, borderRadius: { topLeft: 0, topRight: 0, bottomLeft: 4, bottomRight: 4 }, stack: 'city' },
      { label: 'In-store', data: data.channelByCity.map(c => c.inStore),  backgroundColor: `${COLORS[1]}99`, borderColor: COLORS[1], borderWidth: 1, borderRadius: { topLeft: 4, topRight: 4, bottomLeft: 0, bottomRight: 0 }, stack: 'city' },
    ],
  }
  ...
  const stackedOptions = {
    ...
    plugins: { ...barOptions.plugins, legend: { labels: legendLabels } },
  ...
  return (
    <section id="section3" className="section-anchor space-y-5 fade-in-up">
      ...
      <div className="glass-card p-4 flex items-start gap-3 border-l-2 border-l-cyan-500">
        ...
        <p className="text-sm text-gray-600 dark:text-white/70 leading-relaxed">{data.summary3}</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Revenue by Gender</h3>
          <p className="text-xs text-gray-400 dark:text-white/40 mb-5">Share of total net revenue</p>
          <div className="h-64"><Doughnut data={genderData} options={donutOptions} /></div>
        </div>
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Revenue by Age Group</h3>
          <p className="text-xs text-gray-400 dark:text-white/40 mb-5">Net revenue across customer age bands</p>
          <div className="h-64"><Bar data={ageData} options={barOptions} /></div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Revenue by City</h3>
          <p className="text-xs text-gray-400 dark:text-white/40 mb-5">Top cities ranked by net revenue</p>
          <div className="h-64"><Bar data={cityData} options={barOptions} /></div>
        </div>
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Online vs In-store by City</h3>
          <p className="text-xs text-gray-400 dark:text-white/40 mb-5">Channel split per city</p>
          <div className="h-64"><Bar data={stackedData} options={stackedOptions} /></div>
        </div>
      </div>
    </section>
  )
